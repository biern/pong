YUI.add("simulator", function (Y) {
  /**
   * TODO:
   * 
   */
  var prefix = "simulator:";
  var Simulation = Y.namespace("Pong").GameSimulator =
    Y.Base.create(
    "GameSimulator", Y.Base, [],
    {
	...
	var simulator = function(){
	  ...
	  this._simulationTimer = Y.later(
	    parseInt(this.get("board.interval")),
	    this,
	    simulator,
	    []
	  );
	...
	var timeDelta = ((new Date).getTime() - simulated.get("timestamp")),
	  framesDelta = parseInt(timeDelta / this.get("board.interval"));
	...
	this._bindClient(this.get('client'));
      ...
      _afterClientChange: function(evt){
	var client = evt.newVal;
	this._bindClient(client);
      },
      _bindClient: function(client){
	client.on('server:gameStarted', this.start, this);
	client.on('server:gameFinished', this.stop, this);
	client.on('server:gameSnapshot', this._onGameSnapshot, this);
	client.on('server:gameBoardInfo', this._onGameBoardInfo, this);
	client.on('server:playerInfo', this._onPlayerInfo, this);
	client.on('input:move', this._onPlayerMove, this);
	this.addTarget(client);
      },
      _onGameSnapshot: function(evt, snapshot){
	this.addSnapshot(snapshot);
      },
      _onGameBoardInfo: function(evt, board){
	this.set('board', board);
      },
      _onPlayerInfo: function(evt, player){
	this.set('player', player);
      },
      _onPlayerMove: function(evt, moveData){
	if(this.get('running')){
	...
	player: {
	  value: new Y.Pong.Player()
	}
      }
    }
  );
  Y.log("module loaded", "debug", "simulation");
}, "0.0.0", { requires: ["utils", "snapshot", "paddle", "ball", "player"] });
