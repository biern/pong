YUI.add("connectionhandler", function (Y) {
  /**
   * TODO:
   * Rebuild responses system to be pluggable:
   *   ConnectionHandler.register(target, requests, responses)
   */
  var prefix = "server:";
  var ConnectionHandler = Y.namespace("Pong").ConnectionHandler = Y.Base.create(
	...
	var tOptions = this.get('transportsOptions');
	var socket = this._socket = new io.Socket(host,
	  { port: port,
	    transports: ['websocket', 'flashsocket'],
	    transportOptions: {
	      websocket: tOptions,
	      flashsocket: tOptions
	    },
	    reconnect: true
	  });
	var that = this;
	...
	// Public alias to _makeRequest.
	return this._makeRequest(type, data);
      },
      // Handling events
	...
	allResponses: {
	  value: [
	    // Auth
	    'authRequest', 'authSuccess', 'authFailed', 
	    // Server
	    'serverInfo', 'serverMessage', 'debugMessage', 'pingResponse',
	    'notSupported', 
	    // Player
	    'playerInfo',
	    // Game
	    'gameSnapshot', 'gameStartCount',
	    'gameBoardInfo', 'gameNewRound', 'gameScore',
	    'gameRivalLeft', 'gameStarted', 'gameFinished', 
	    // Lobby
	    'lobbyPlayerJoined', 'lobbyPlayerLeft', 'lobbyPlayerUpdated',
	    'lobbyEntered', 'lobbyChatMessage', 'lobbyPlayersList'],
	  readOnly: true
	},
	eventNames: {
