YUI.add("client", function (Y) {
  var Client = Y.namespace("Pong").Client = Y.Base.create(
	...
	this.on("server:gameBoardInfo", function(evt, board){
	  // this._simulation.set("board", board);
	  this._renderer.set("board", board);
	});
	...
	this.on("server:serverInfo", function(evt, data){
	  this._connectionHandler._makeRequest("lobbyJoin", "Test room");
	});
      },
	...
	this.on("*:move", function(evt, moveData){
